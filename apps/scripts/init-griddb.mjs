...
const fileEnvironment = readEnvironmentFile(environmentPath);
...
const python = configuredPython
  ? (isAbsolute(configuredPython) ? configuredPython : resolve(bridgeRoot, configuredPython))
  : defaultPython;
...
await runOrExit(python, ['python/init_schema.py'], {
  cwd: bridgeRoot,
  env: environment,
});
